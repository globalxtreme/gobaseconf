...
func (ex *Excel) NewFile() *Excel {
	...
	for key, sheet := range ex.Sheets {
		...
		for _, property := range ex.Properties[key] {
			addRow(sheet, ex.File, rKey, property)
			rKey++
		}
...
func (ex *Excel) MergeCells(cells ...string) *Excel {
	...
	for _, cell := range cells {
		firstCell, secondCell := splitCells(cell)
...
func (ex *Excel) SetWidthCols(cWidths []ColWidth) *Excel {
	...
	for _, cWidth := range cWidths {
		firstCell, secondCell := splitCells(cWidth.Cells)
		for _, sheet := range ex.Sheets {
...
func (ex *Excel) Save(path string, filename string) error {
	...
	if ex.IsPublic {
		storagePath = helpers.SetStorageAppPublicDir(path)
	} else {
		storagePath = helpers.SetStorageAppDir(path)
	}

	helpers.CheckAndCreateDirectory(storagePath)
...
func splitCells(cells string) (string, string) {
...
func addRow(sheet string, f *excelize.File, rowIndex int, data []interface{}) {
	columnIndex := 1
